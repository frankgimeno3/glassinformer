...
import {fetchNewTokens, verifyIdToken, verifyAccessToken} from "@/app/server/features/authentication/AuthenticationService.js";
...
export async function proxy(request) {
    ...
    /** Redirect to Unlogged Home and clear auth cookies to prevent redirect loop (ERR_TOO_MANY_REDIRECTS)
     * when LastAuthUser exists but tokens are expired/missing. */
    const goUnloggedHome = (cookieKeys = null) => {
        if (pathname === '/') {
            return response;
        }   
    ...
    // Allow public auth routes without redirecting to /
    if (isPublicPath) {
        return response;
    }

    const username = request.cookies.get(lastAuthUserKey)?.value;
    if (!username) return goUnloggedHome(null);
    ...
    if (!refreshToken) {
        return goUnloggedHome(cookieKeys);
    }
